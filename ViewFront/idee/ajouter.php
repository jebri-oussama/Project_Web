...
$ctl=new IdeeC(); $tCtl=new ThematiqueC(); $themes=$tCtl->all(); $errors=[];
if($_SERVER['REQUEST_METHOD']==='POST'){ if($ctl->store($_POST,$errors)){ header('Location: '.app_url('ViewFront/idee/afficher.php')); exit; } }
$title="Nouvelle idée";
ob_start(); ?>
<section class="card reveal" data-tilt>
  <h2 style="margin:0 0 8px">Proposer une idée</h2>
  ...
  <form method="post" class="grid two" novalidate>
    ...
    <label style="position:relative">
      ...
      <input style="width:100%;padding:28px 12px 12px;border:1px solid var(--stroke);border-radius:12px;background:var(--panel)" type="text" name="titre" value="<?= htmlspecialchars($_POST['titre'] ?? '') ?>" required>
    ...
    <label style="position:relative">
      ...
      <select style="width:100%;padding:28px 12px 12px;border:1px solid var(--stroke);border-radius:12px;background:var(--panel)" name="id_thematique" required>
        ...
        <?php $th=$_POST['id_thematique']??''; foreach($themes as $t){ $sel=($th!=='' && (int)$th===(int)$t['id'])?'selected':''; echo '<option value="'.(int)$t['id'].'" '.$sel.'>'.htmlspecialchars($t['titre']).'</option>'; } ?>
      </select>
    ...
    <label class="grid" style="grid-column:1/-1;position:relative">
      ...
      <textarea id="desc" style="width:100%;min-height:180px;padding:28px 12px 12px;border:1px solid var(--stroke);border-radius:12px;background:var(--panel)" name="description" maxlength="1000" required><?= htmlspecialchars($_POST['description'] ?? '') ?></textarea>
    ...
    <div></div>
    <button class="btn primary" type="submit" onclick="toast('Idée enregistrée')">Enregistrer</button>
  </form>
</section>
<script>
const desc=document.getElementById('desc'), cnt=document.getElementById('cnt');
function upd(){ cnt.textContent=desc.value.length } upd(); desc.addEventListener('input',upd);
</script>
